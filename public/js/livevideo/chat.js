...
function sendMessage(options) {
    ...
    function  doSend(messageData,data){
        ...
        $.post('/chat/send/' + window.D.roomId, messageData, function(text, status) {
            if (text && text.id) {
                if ($('#js-chat-messages-' + text.id).length > 0) {
                    return;
                }
                $chat = $('#js-chat-messages-' + id);
                if (text.is_audited) {
                    $chat.find('.js-chat-checkbox').show();
                    $chat.find('.js-chat-checkbox').attr('value', text.id);
                }
                $chat.find('.chat-message-delete-btn').show();
                $chat.find('.chat-message-delete-btn').attr('data-id', text.id)
                if (!text.is_audited) {
                    $chat.find('.chat-message-audit-btn').show();
                    $chat.find('.chat-message-audit-btn').attr('data-id', text.id)
                }
                $chat.attr('data-msgid', text.id);
                $chat.attr('id', 'js-chat-messages-' + text.id);
            }else if(text && text.code == 133){
                if(text.guest_chat_cue){
                    dialog({title:"提示",content:text.msg,quickClose: true}).show();
                }
                
            }
            if(text && text.chat_ts){
                window.D.USER.chatIntervalCount = text.chat_ts;
            }
        });
        data.id = id;
        outputChatMessage($CHAT_MESSAGE.render(data), true)
